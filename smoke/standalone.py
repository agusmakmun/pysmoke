...
@click.command()
@click.option(
    "-c", "--config", "config_path", default="config/app.conf", help="Config file path"
)
@click.option(
    "-s", "--source", "source_path", default="tests", help="Tests folder path"
)
@click.option(
    "-f", "--filter", "filtered_class", default=None, help="Run just this test"
)
